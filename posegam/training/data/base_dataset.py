...
class BaseDataset(Dataset):
    """
    Base dataset class for PoseGAM and VGGSfM training.
    ...
    def process_one_image(
        ...
        image = np.copy(image)
        base_color = np.copy(base_color)  # base color image
        ...
        image, base_color, normal, depth_map, intri_opencv, track = crop_image_depth_and_intrinsic_by_pp(
            image, base_color, normal, depth_map, intri_opencv, aug_size, track=track, filepath=filepath,
        )
        ...
        if self.rescale:
            image, base_color, normal, depth_map, intri_opencv, track = resize_image_depth_and_intrinsic(
                image, base_color, normal, depth_map, intri_opencv, target_shape, original_size, track=track,
        ...
        image, base_color, normal, depth_map, intri_opencv, track = crop_image_depth_and_intrinsic_by_pp(
            image, base_color, normal, depth_map, intri_opencv, target_shape, track=track, filepath=filepath, strict=True,
        )

        # Apply 90-degree rotation if needed
        if rotate_to_portrait:
            raise NotImplementedError("90-degree rotation is not implemented yet.")
