...
def composed_dataset_collate_fn(batch):
    """
    Custom collate function for ComposedDataset that handles batching of sequences
    with different data types including tensors, lists, and metadata.
    
    Args:
        batch: List of sample dictionaries from ComposedDataset.__getitem__
        
    Returns:
        dict: Batched dictionary with all fields properly collated
    """
    if not batch:
        raise ValueError("Received an empty batch in collate function.")
    ...
    for key in sample_keys:
        ...
        if key == "seq_name":
            # Keep sequence names as a list of strings
            collated_batch[key] = values
            
        elif key in ["ids", "camera_mask"]:
            # These are 1D tensors or arrays that can be stacked directly
            collated_batch[key] = torch.stack(values, dim=0)
            
        elif key in ["images", "base_colors", "extrinsics", "intrinsics"]:
            # These are multi-dimensional tensors that should be stacked
            collated_batch[key] = torch.stack(values, dim=0)
            
        elif key in ["depths", "cam_points", "world_points", "point_masks", "initial_sonata_num"]:
            # These might be empty lists for some samples, handle carefully
            if all(isinstance(v, list) and len(v) == 0 for v in values):
                # All samples have empty lists
                collated_batch[key] = []
            elif all(torch.is_tensor(v) and v.numel() > 0 for v in values):
                # All samples have non-empty tensors
                try:
                    collated_batch[key] = torch.stack(values, dim=0)
                ...
                # Mixed case - some empty, some with data, or different types
                collated_batch[key] = values
                
        elif key == "point_sonata":
            # Handle sonata data - might be empty lists or have special structure
            if all(isinstance(v, list) and len(v) == 0 for v in values):
                collated_batch[key] = []
            else:
                # Keep as list since sonata data might have complex structure
                collated_batch[key] = sonata.data.collate_fn(values)
        else:
            # For any other keys, try to stack if they're tensors, otherwise keep as list
...
class DynamicTorchDataset(ABC):
    def __init__(
        ...
        # Use the custom collate function by default if none is provided
        self.collate_fn = collate_fn if collate_fn is not None else composed_dataset_collate_fn
