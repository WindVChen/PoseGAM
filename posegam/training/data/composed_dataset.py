...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        base_colors = torch.from_numpy(np.stack(batch["base_colors"]).astype(np.float32)).contiguous()
        base_colors = base_colors.permute(0,3,1,2).to(torch.get_default_dtype()).div(255)

        # Convert other data to tensors with appropriate types
        depths = torch.from_numpy(np.stack(batch["depths"]).astype(np.float32)) if batch["depths"] != [] else []
        ...
        cam_points = torch.from_numpy(np.stack(batch["cam_points"]).astype(np.float32)) if batch["cam_points"] != [] else []
        world_points = torch.from_numpy(np.stack(batch["world_points"]).astype(np.float32)) if batch["world_points"] != [] else []
        point_masks = torch.from_numpy(np.stack(batch["point_masks"])) if batch["point_masks"] != [] else [] # Mask indicating valid depths / world points / cam points per frame
        ids = torch.from_numpy(batch["ids"])    # Frame indices sampled from the original sequence
        camera_mask = torch.from_numpy(batch["camera_mask"])

        point_sonata = batch["point_sonata"] if batch["point_sonata"] != [] else []
        initial_sonata_num = torch.from_numpy(np.array(batch["initial_sonata_num"]).astype(np.int32)) if batch["initial_sonata_num"] != [] else []
        ...
        sample = {
            "seq_name": seq_name,
            "ids": ids,
            'camera_mask': camera_mask,  # Mask indicating which frames have valid camera poses
            "images": images,
