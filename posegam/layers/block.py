...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, context: Tensor = None, context_pos=None) -> Tensor:
        def attn_residual_func(x: Tensor, pos=None, context: Tensor = None, context_pos=None) -> Tensor:
            if self.attn.__class__ == CrossAttention:
                return self.ls1(self.attn(self.norm1(x), context=context, pos=pos, context_pos=context_pos))
            else:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            if self.attn.__class__ == CrossAttention:
                x = x + attn_residual_func(x, pos=pos, context=context, context_pos=context_pos)
            else:
