...
def init_render(engine='CYCLES', resolutionx=512, resolutiony=512, geo_mode=False, force_cycles=False):
...
def init_lighting(light_mode='standard'):
    """Set up scene lighting.

    Args:
        light_mode:
            'standard' : a key point light plus top/bottom area lights.
            'bright'   : a grid of point lights around the object, giving a brighter,
    ...
    if light_mode == 'bright':
        # Grid of point lights around the object -> brighter, even illumination.
        light_locations = []
        for x in [-5, 5]:
            for y in [-5, 5]:
                for z in [-5, 5]:
        ...
        for i, location in enumerate(light_locations):
            light_name = f"POINT_Light_{i}"
            light = bpy.data.objects.new(light_name, bpy.data.lights.new(light_name, type="POINT"))
    ...
    # 'standard': key light + top/bottom area lights.
...
def main(arg):
    ...
    # Initialize context
    init_render(engine=arg.engine, resolutionx=arg.resolutionx, resolutiony=arg.resolutiony, geo_mode=arg.geo_mode, force_cycles=arg.use_environment_map)
    ...
    if arg.use_environment_map:
        ...
        init_lighting(light_mode=arg.light_mode)
    ...
    to_export = {
        ...
        "image_width": arg.resolutionx,
        "image_height": arg.resolutiony,
        "frames": []
    }
    ...
    for i, view in enumerate(views):
        ...
        # Set camera focal length using both fovx and fovy
        fovx = view['fovx']
        fovy = view['fovy']

        # Calculate focal length based on fovy (vertical field of view)
        # Using sensor height as reference (32mm as set in init_camera)
        cam.data.lens = cam.data.sensor_height / (2 * np.tan(fovy / 2))
        
        # Adjust sensor width to match the desired fovx
        # focal_length = sensor_width / (2 * tan(fovx / 2))
        # sensor_width = focal_length * 2 * tan(fovx / 2)
        cam.data.sensor_width = cam.data.lens * 2 * np.tan(fovx / 2)
        ...
        metadata = {
            ...
            "camera_angle_x": view['fovx'],
            "camera_angle_y": view['fovy'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resolutionx', type=int, default=512, help='Resolution of the images.')
    parser.add_argument('--resolutiony', type=int, default=512, help='Resolution of the images.')
    parser.add_argument('--engine', type=str, default='CYCLES', help='Blender internal engine for rendering. E.g. CYCLES, BLENDER_EEVEE, ...')
    parser.add_argument('--light_mode', type=str, default='standard', choices=['standard', 'bright'], help="Scene lighting: 'standard' (key + top/bottom area lights) or 'bright' (denser point-light grid).")
