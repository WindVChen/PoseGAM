...
def foreach_instance(metadata, output_dir, func, max_workers=None, desc='Processing objects', timeout=14400) -> pd.DataFrame:
    import os
    ...
    try:
        with ThreadPoolExecutor(max_workers=max_workers) as executor, \
            ...
            def worker(metadatum):
                ...
                try:
                    if local_path.startswith('raw/github/repos/'):
                        ...
                        file = os.path.join(output_dir, 'watertight_meshes', sha256, 'mesh.glb')
                        record = func(file, sha256)
            ...
            # Submit tasks and process with timeout
            futures_to_metadatum = {executor.submit(worker, item): item for item in metadata}
            
            for future in as_completed(futures_to_metadatum, timeout=timeout):
                metadatum = futures_to_metadatum[future]
                sha256 = metadatum['sha256']
                try:
                    future.result(timeout=timeout)
                except TimeoutError:
                    print(f"Timeout processing object {sha256} after {timeout} seconds, skipping...")
                except Exception as e:
                    print(f"Exception processing object {sha256}: {e}")
                finally:
                    pbar.update(1)
    except:
