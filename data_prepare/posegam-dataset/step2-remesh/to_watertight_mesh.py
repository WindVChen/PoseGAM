...
def texture_mesh(textured_mesh_path: str, non_textured_mesh_path: str, output_folder: str = None) -> None:
    ...
    # Import the textured mesh (reference)
    if not textured_mesh_path.lower().endswith('.glb'):
        print(f'[WARNING] Expected GLB format for textured mesh: {textured_mesh_path}')
        return
        
    bpy.ops.import_scene.gltf(filepath=textured_mesh_path)
    ...
    if not non_textured_mesh_path.lower().endswith('.obj'):
        print(f'[WARNING] Expected OBJ format for non-textured mesh: {non_textured_mesh_path}')
        return
        
    bpy.ops.wm.obj_import(filepath=non_textured_mesh_path)
...
def remesh(grid_xyz, grid_size, mesh_path, remesh_path, resolution, use_pcu):
    ...
    mesh.export(os.path.join(os.path.dirname(remesh_path), 'mesh_scaled.glb'))
    ...
    texture_mesh(textured_mesh_path=os.path.join(os.path.dirname(remesh_path), 'mesh_scaled.glb'), non_textured_mesh_path=remesh_path, output_folder=output_folder)

    # delete mesh_path.replace('.glb', '_scaled.glb') and remesh_path
    # if os.path.exists(os.path.join(os.path.dirname(remesh_path), 'mesh_scaled.glb')):
    #     os.remove(os.path.join(os.path.dirname(remesh_path), 'mesh_scaled.glb'))
...
def main(resolution, meshes_paths, remesh_target_path, use_pcu, rank_id) -> None:
    ...
    for mesh_path in tqdm(meshes_paths, desc=f"Processing meshes (rank {rank_id})"):
        ...
        if os.path.exists(remesh_path.replace('obj', 'glb'))==False:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="128",
        type= int,
        help="UDF grid resolution for the watertight remeshing (higher = finer mesh, slower). Default: 128.",
    )
    parser.add_argument(
        "--json_file_path",
        type= str,
        help="Path to the JSON file listing the converted_meshes .glb paths (produced by detect_path.py).",
        default="/ibex/tmp/TRELLIS-500K/ObjaverseXL_sketchfab/converted_meshes/mesh_path.json"
    )
    parser.add_argument(
        "--remesh_target_path",
        type= str,
        help="Directory to save the remeshed (watertight) meshes.",
        default="/ibex/tmp/TRELLIS-500K/ObjaverseXL_sketchfab/watertight_meshes"
    )
    parser.add_argument(
        "--use_pcu",
        action='store_false',
